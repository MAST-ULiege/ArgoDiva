...
MLDdf <- ldply(as.list(1:length(unique(densitydf$id))), function(i){
  ...
  data.frame(rho_surf = rho_surf, MLD = MLD, juld = tmp$juld[1])
})
...
gaussiandf <- ldply(as.list(1:length(unique(profiledf$id))), function(i){
  ...
  if (tmp$juld[1] == profilechiant[1] | tmp$juld[1] == profilechiant[2]
      | tmp$juld[1] == profilechiant[3]){
    dz <- 10
  }
  
  else if(tmp$juld[1] == profilechiant[4]){
    Zdemi <- 30
  }
...
sigmoidf <- ldply(as.list(1:length(unique(profiledf$id))), function(i){
    ...
    if (tmp$juld[1] == profilechiantBIS[1] | tmp$juld[1] == profilechiantBIS[2]
        | tmp$juld[1] == profilechiantBIS[3]
        | tmp$juld[1] == profilechiantBIS[4]){
      s <- -0.1
    }

    else if(tmp$juld[1] == profilechiantBIS[5]){
      Zdemi <- 30
    }
...
# QUENCHING CORRECTION NON BASÉ SUR LA PAR car pas disponible sur les 4 flotteurs. 
# 
# Calculate of MLD (crit diff dty 0.03)
MLD<-NA

if(length(sigma[!is.na(sigma)==TRUE])>=2) {
  sigmaSurface<-NA
  sigmaSurface <- approx(dep_sigma,sigma,10)$y # identification de la densit? ? 10m ("surface", pour variations journali?res)
...
QuenchingCor2 <- function(fluo,Depth,MLD) {
