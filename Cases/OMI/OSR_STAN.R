...
##########################
  ...
  pdf(paste("~/Desktop/PAPERS_UNDER_WORK/OSR3/figure/CMEMS_OSR3_VOC_CCC_allin_NN_linear_2107.pdf",sep=""),
      height = 7,width=7*1.68)
  grid.arrange(leftband, central,  leg, lowband,
               ncol = 2, nrow = 2,
  ...
  postscript(file="~/Desktop/PAPERS_UNDER_WORK/OSR3/figure/CMEMS_OSR3_Sec3.5_F3.eps", 
             width  = 8.27,
             height = 8.27 /1.68,
             family = "Times")
  grid.arrange(leftband, central,  leg, lowband,
...
########
  ...
  plist<-lapply(c("Z20","R20","VOC"), function(vavar) {
    ggplot(subset(trendf,Nprofs>Nprofmin&Year>yearmin&VAR==vavar&Year<yearmax)
      ...
      geom_smooth(data = subset(finaldffroploc,time>2012), aes_string(x="time",y=vavar, fill="Set", color="Set"))+
  ...
  YBRDF<-data.frame(xmin=YBREAKS[-c(1,6)],xmax=YBREAKS[-c(1,2)],ymin=YLIM2[1],ymax=YLIM2[2] )
  
  ti+geom_rect(data=YBRDF,aes(xmin=xmin,ymin=ymin,xmax=xmax,ymax=ymax),fill=brewer.pal(4,PAL),alpha=.2)
  
  basedir<-"~/Desktop/CODES/R/VOX/"
  #pdf(paste(basedir,"inter_trendRECT.pdf",sep=""),width=5*2.2,heigh=5)
  ggplot(YBRDF)+geom_rect(aes(xmin=xmin,ymin=ymin,xmax=xmax,ymax=ymax),fill=brewer.pal(4,PAL),alpha=.2,names=c(1,2,3,4))+theme_bw()+ylab("d")+theme( panel.grid.major = element_blank(), panel.grid.minor = element_blank())
  #dev.off()
