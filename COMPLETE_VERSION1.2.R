...
# 
# tmp_npq <- quenching_correction(tmp$fluo, tmp$depth, MLDdf$MLD[i])
# tmp2 <- cbind(tmp, tmp_npq)
# depthindex <- which.min(tmp2$depth <= 100)
# tmp2 <- tmp2[1:depthindex,]
# ggplot(tmp2, aes(x = tmp_npq, y = depth)) + geom_path() + scale_y_reverse() +
#   xlab("Fluorescence (RFU)") + ylab("Depth (m)") 
# 
